...
dat %>% #View 
  filter(ai=="Azoxystrobin") %>% #cyproconazole
  # filter(ai=="Cyproconazole") %>% #cyproconazole
  ggplot(aes(x=ai, y=(x3+x4)/n_pods)) +
  # geom_boxplot()+
  geom_text(aes(label=pot),  position = position_dodge(width = 1))+  
